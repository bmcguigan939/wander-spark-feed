...
export function CategoryChips({
  ...
  return (
    <div className="pointer-events-auto flex gap-1.5 overflow-x-auto pb-1 pr-3 snap-x [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      {ITEMS.map(({ value: v, label, Icon }) => {
        ...
        return (
          <button
            key={v}
            onClick={() => onChange(v)}
            className={`flex flex-shrink-0 snap-start items-center gap-1 rounded-full border px-2.5 py-1.5 text-[11px] font-semibold transition ${
              active
