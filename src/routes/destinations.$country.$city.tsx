...
function CityPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["destination", country, city],
    queryFn: () => getDestination({ data: { country, city } }),
  });
  ...
  return (
    <MobileShell>
      <div className="px-4 pt-6">
        ...
        <p className="mt-1 text-sm text-muted-foreground">{country} · {videos.length} video{videos.length === 1 ? "" : "s"}</p>
