...
function TermsPage() {
  return (
    <LegalPage title="Terms of Service" updated="November 2026">
      ...
      <h2 id="activity-pricing">4a. Activity pricing and reseller comparisons</h2>
      <p>Some activity and tour deals on Travidz use an <strong>operator-direct pricing model</strong>. The operator sets their own per-person price (the same price they sell at on their own website) and Travidz adds an <strong>11% booking fee</strong> on top to cover secure checkout, customer support, and the creator rewards pool. The total you pay on Travidz is the operator's price plus 11%.</p>
      <p>Where we show a "cheaper than other resellers" badge, the comparison is against <strong>third-party resellers</strong> — for example GetYourGuide, Viator, Klook, Tiqets, Musement and similar platforms that resell activities with their own markup (commonly 25–40%). We do <strong>not</strong> compare against the operator's own website, and we make no claim about it. The operator may sell the same activity direct on their own site.</p>
      <p>Price-match guarantees on activity deals are scoped to third-party resellers only. If you find the same activity on another reseller for less on the same date and party size, contact us and we'll match the third-party price minus our 11% booking fee. Direct prices on operator sites are not eligible for matching.</p>
      <h2>5. Bookings, payment and refunds</h2>
      <p>When you complete checkout for a bookable deal, you'll receive a confirmation email and a booking record in your account. Payment is processed by <strong>Stripe</strong>; we never see or store your full card details. For bookings of third-party hotels, tours and activities, Stripe automatically splits the charge at capture — the operator or hotel's share is transferred directly to their connected Stripe account, and Travidz retains only its platform fee.</p>
