...
function fallbackInviteDraft(args: {
  ...
  return {
    ...
    body:
      ...
      followerLine +
      socialsBlock +
      `\nOne setup, then it's just a tap. Once you accept, any other Travidz creator who wants to feature you can be added with a single tap from your dashboard — no new forms, no new agreement.\n` +
      `\nMore creators means more global reach: every additional creator puts your listing in front of their audience, in their language, in the cities they travel to. No cap, no extra cost.\n` +
...
export const draftInviteEmail = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<Draft> => {
    ...
    const system =
      ...
      "After step (3), include one short paragraph explaining that the setup is a one-time thing: once the business accepts, any other Travidz creator who wants to feature them later can be added with a single tap from the business dashboard — no new forms, no new agreement. Then add one short sentence noting that more accepted creators = more global reach for the business (different audiences, different languages, different cities), with no cap and no extra cost beyond the same commission on bookings. " +
