...
s.addText("Raising £2.5M SAFE  ·  18-month runway  ·  UK → EU-5",{x:W-6.2,y:0.1,w:5.95,h:0.3,fontSize:11,bold:true,color:CYAN,align:"right",fontFace:"Calibri"});
...
mcell(1,"SAM","£23.2B / £82.9B","Phocuswright · GWI · Skift");
mcell(2,"Y5 SOM","£444M · £20.7M","GBV · net rev · 4.65% take");
...
s.addText("Seed · 18-mo runway",{x:ax+1.3,y:BY+0.42,w:cw-1.5,h:0.3,fontSize:9,color:MUTED,fontFace:"Calibri"});
...
s.addText([
  ...
  {text:" market booked blind. Gen-Z & Millennials discover trips on creators and TikTok, then jump to OTAs that pay creators nothing.",options:{color:INK}}
],{x:LX+0.2,y:lyP+0.45,w:LW-0.4,h:cardH-0.55,fontSize:10,fontFace:"Calibri",valign:"top"});

panel(LX,lyT,LW,cardH,"TRACTION");
[["Waitlist","3,200+"],["Creator LOIs","85"],["Supply partners","12"]].forEach(([l,v],i)=>{
  const ry=lyT+0.5+i*0.34;
  s.addText(l,{x:LX+0.2,y:ry,w:LW-0.4,h:0.3,fontSize:10,color:MUTED,fontFace:"Calibri"});
...
[["Founder One","CEO · ex-Booking.com"],
 ["Founder Two","CTO · ex-TikTok Shop"],
 ["Founder Three","CPO · ex-Airbnb"]].forEach(([n,b],i)=>{
  const ry=lyTm+0.5+i*0.34;
  s.addShape("ellipse",{x:LX+0.2,y:ry+0.06,w:0.16,h:0.16,fill:{color:CYAN},line:{color:CYAN}});
  s.addText(n,{x:LX+0.45,y:ry,w:1.3,h:0.28,fontSize:9.5,bold:true,color:INK,fontFace:"Calibri"});
  s.addText(b,{x:LX+1.65,y:ry,w:LW-1.8,h:0.28,fontSize:9,color:MUTED,fontFace:"Calibri"});
});
...
s.addText([
  ...
  {text:" is a shoppable travel feed. Creators post trips → travellers book in-app → creators earn on every booking. One feed. One checkout. One ledger.",options:{color:INK}}
],{x:RX+0.2,y:ryS+0.45,w:RW-0.4,h:cardH-0.55,fontSize:10,fontFace:"Calibri",valign:"top"});

panel(RX,ryB,RW,cardH,"BUSINESS MODEL");
["• 4-7% take-rate (stays · tours · experiences)",
 "• Creator rev-share 30-50% of net",
 "• Tiered creator subs · brand partnerships",
 "• Blended Y5 take 4.65%"].forEach((t,i)=>{
...
[["PROVE","M0-18 · UK · 2.4k cr · £44M GBV"],
 ["SCALE","M18-44 · EU-5 · 14k cr · £259M"],
 ["DEFEND","M44-60+ · Moat · 24k cr · £444M"]].forEach(([p,desc],i)=>{
  ...
  s.addText(desc,{x:RX+0.95,y:ry,w:RW-1.1,h:0.3,fontSize:9,color:INK,fontFace:"Calibri"});
});

await pres.writeFile({fileName:"/mnt/documents/Travidz_Elevator_Pitch_v2.pptx"});
console.log("OK");
